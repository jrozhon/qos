...
| `qos-04` | PSNR / SSIM image quality metrics | PIL, scipy (still on Poetry `pyproject`, not yet migrated to uv) |
...
`qos-04/pyproject.toml` is a legacy Poetry file; converting it to the `[project]` table used by 01–03 is pending (see commit history "moved to uv").
...
- `lib/params.py` (identical in 01–03) holds the shared matplotlib `rc_params` dict and `colors` palette; the target content is in `STYLE.md` §5.1. If you touch one, mirror it in the other.
